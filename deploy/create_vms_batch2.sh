#!/usr/bin/env bash
# Batch 2: create one g2-standard-32 (1x NVIDIA L4) VM with 200GB disk in each
# of the 6 new test projects (282-287). Same logic as create_vms.sh.
# Seeds PREFERRED_ZONE=europe-west4-c so the new VMs prefer the same zone as the
# existing 4-node cluster (281/288/289/290) when capacity allows.
set -u
export PATH=/opt/google-cloud-sdk/bin:$PATH

ACCOUNTS=(
  "<account-282> ebim26ham-282"
  "<account-283> ebim26ham-283"
  "<account-284> ebim26ham-284"
  "<account-285> ebim26ham-285"
  "<account-286> ebim26ham-286"
  "<account-287> ebim26ham-287"
)
...
# Seed with the existing cluster's zone so new nodes colocate when possible.
PREFERRED_ZONE="europe-west4-c"
declare -A RESULTS
